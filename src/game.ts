...
export default class Game {
    ...
    attemptSelectDot(worldX: number, worldY: number) {
        let closestDotIndex = -1
        let closestDistanceSquared = Infinity
        for (let i = 0; i < this.dots.length; i++) {
            const dot = this.dots[i]!
            if (dot.fixed) continue
    ...
    onMouseMove(event: MouseEvent) {
        if (this.selectedDotIndex === -1) return
    ...
    isOverlap(line1: Line, line2: Line) {
        if (line1.dotIndex1 === line2.dotIndex1) return false
        if (line1.dotIndex1 === line2.dotIndex2) return false
        if (line1.dotIndex2 === line2.dotIndex1) return false
        if (line1.dotIndex2 === line2.dotIndex2) return false
    ...
    renderOverlaps() {
        const isOverlapping = new Array<boolean>(this.lines.length).fill(false)
    ...
    worldToScreenY(worldY: number) {
        return this.center.y + worldY * this.scale
    }
    screenToWorldX(screenX: number) {
        return (screenX - this.center.x) / this.scale
    }
    screenToWorldY(screenY: number) {
